...
class AppStore extends Component{
  state={activeAppId:tabsList[0].tabId,
  searchInput:''}

  updateActiveAppId=(tabId)=>{
    this.setState({activeAppId:tabId})
  }

  getFilteredApps=(appsList)=>{
    const {activeAppId}=this.state
    const filteredApps=appsList.filter(app=>(app.category===activeAppId))
  return filteredApps
  }
  ...
  getSearchResults=()=>{
    ...
    const searchResults=appsList.filter(e=>(e.appName.toLowerCase().includes(searchInput.toLowerCase())))
  return searchResults
  }

  render(){
    const {activeAppId,searchInput}=this.state
    ...
    return(
      ...
      <input type="search" className="searchbar" onChange={this.onChangeSearchInput} placeholder="Search" value={searchInput}/>
      ...
      {tabsList.map(each=>(
        <TabItem key={each.tabId} tabDetails={each} updateActiveAppId={this.updateActiveAppId} isActiveTabId={activeAppId===each.tabId}/>
      ))}
      </ul>
